...
class Wow_Admin_Class {
	...
	static function option( $arg ) {
		...
		elseif ( $type == 'text' || $type == 'number' || $type == 'hidden' ) {
			...
			$field = '<input name="' . $name . '" type="' . $type . '" value="' . $val . '" id="' . $id . '"' . $func .
	...
	public function add_admin_page() {
		...
		add_submenu_page( 'wow-company', $this->plugin_name . ' version ' . $this->plugin_version, $this->plugin_menu,
			'manage_options', $this->plugin_slug, array(
				$this,
				'plugin_page',
			) );
	}
...
//    wp_enqueue_style( $this->plugin_slug . '-admin', $this->plugin_url . 'assets/css/admin-style.min.css', false, $this->plugin_version );
		wp_enqueue_style( $this->plugin_slug . '-admin', $this->plugin_url . 'assets/css/admin-style.css', false,
			$this->plugin_version );

		wp_enqueue_style( 'wow-plugin-fontawesome',
			$this->plugin_url . 'assets/vendors/fontawesome/css/fontawesome-all.min.css', null, '5.6.3' );
		// iconpicker
		wp_enqueue_script( 'wow-plugin-fonticonpicker',
			$this->plugin_url . 'assets/vendors/fonticonpicker/fonticonpicker.min.js', array( 'jquery' ) );
		wp_enqueue_style( 'wow-plugin-fonticonpicker',
			$this->plugin_url . 'assets/vendors/fonticonpicker/css/fonticonpicker.min.css' );
		wp_enqueue_style( 'wow-plugin-fonticonpicker-darkgrey',
			$this->plugin_url . 'assets/vendors/fonticonpicker/fonticonpicker.darkgrey.min.css' );
		...
		wp_enqueue_script( 'wp-color-picker-alpha', $this->plugin_url . 'assets/js/wp-color-picker-alpha.min.js',
			array( 'wp-color-picker' ) );
//    wp_enqueue_script( $this->plugin_slug . '-admin', $this->plugin_url . 'assets/js/admin-script.js', array( 'jquery' ), $this->plugin_version );
		wp_enqueue_script( $this->plugin_slug . '-admin', $this->plugin_url . 'assets/js/admin-script.min.js',
			array( 'jquery' ), $this->plugin_version );
	...
	public function action_links( $actions, $plugin_file ) {
		if ( false === strpos( $plugin_file, plugin_basename( $this->plugin_file ) ) ) {
			return $actions;
		}
		$settings_link =
			'<a href="admin.php?page=' . $this->plugin_slug . '">' . __( 'Settings', $this->plugin_pref ) . '</a>';
	...
	public function rate_us( $footer_text ) {
		...
		if ( $typenow == $this->plugin_slug ) {
			$rate_text = sprintf( __( 'Thank you for using <a href="%1$s" target="_blank">' . $this->plugin_name .
			                          '</a>! Please <a href="%2$s" target="_blank">rate us on WordPress</a>',
				$this->plugin_pref ), $this->plugin_home_url,
				'https://wordpress.org/support/plugin/modal-window/reviews/?filter=5' );
	...
	public function plugin_check() {
		if ( isset( $_POST[ $this->plugin_slug . '_nonce' ] ) ) {
			if ( ! empty( $_POST ) &&
			     wp_verify_nonce( $_POST[ $this->plugin_slug . '_nonce' ], $this->plugin_slug . '_action' ) &&
	...
	public function save_data() {
		...
		if ( isset( $_POST['submit'] ) ) {
			if ( absint( $_POST['add'] ) == '1' ) {
				...
				header( 'Location:admin.php?page=' . $page . '&tab=add_new&act=update&id=' . $tool_id .
				        '&info=update' );
				exit;
			}
		}
	...
	function general_admin_notice() {
		echo '<div class="notice notice-warning wow-plugin-notice is-dismissible">
				<p><b>' . $this->plugin_name .
		     '</b>! Please check all your modal windows and make changes if necessary. Please note that the Font Awesome icons and styles of close button have been updated. . <a href="' .
		     admin_url() . 'admin.php?page=' . $this->plugin_slug . '">Go to the ' . $this->plugin_name . ' page</a>. Sorry for the inconvenience.</p>
				</div>';

	}

	function admin_notice_assets() {
		wp_enqueue_script( 'wow-notice-update', $this->plugin_url . 'assets/js/notice-update.js', array( 'jquery' ),
			'1.0', true );
	}
