...
class WPBForWPbakery_Product_Add_To_Cart{
    ...
    public function render_shortcode( $atts, $content = null ) {
        ...
        ob_start();
        $unique_class = uniqid('summary entry-summary wpbforwpbakery_product_price_');
        $output = '<style>';
        $output .= ".$unique_class .single_add_to_cart_button{background-color: {$button_bg_color}; color: {$button_text_color};  }";
        $output .= ".$unique_class .single_add_to_cart_button:hover{background-color: {$button_hover_bg_color}; color: {$button_hover_text_color} }";
        $output .= '</style>';
