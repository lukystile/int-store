...
if ($image_as_popup) {
	$image_as_popup = ' jt-popup-image ';
} else {
	$image_as_popup = '';
}
if ($title_image_caption) {
	$caption_class = ' title_image_caption ';
	$caption_class_image = ' hidden ';
} else {
	$caption_class = '';
	$caption_class_image = '';
}
...
if ( $link ) {
	...
	if ($title_image_caption) {
		$html = '<div class="jt-fig-cont '. $have_popup .'"><figure><a ' . vc_stringify_attributes( $a_attrs ) . ' class="' . $wrapperClass . '">' . $img['thumbnail'] . '</a></figure><span><em>'. $title .'</em></span></div>';
	} else {
		$html = '<a ' . vc_stringify_attributes( $a_attrs ) . ' class="' . $wrapperClass . '">' . $img['thumbnail'] . '</a>';
	}
} else {
	if ($title_image_caption) {
		$html = '<div class="jt-fig-cont '. $have_popup .'"><figure><div class="' . $wrapperClass . '">' . $img['thumbnail'] . '</figure><span><em>'. $title .'</em></span></div>';
	} else {
		$html = '<div class="' . $wrapperClass . '">' . $img['thumbnail'] . '</div>';
	}
}

$class_to_filter = 'wpb_single_image wpb_content_element vc_align_' . $alignment . ' ' . $this->getCSSAnimation( $css_animation ) . $image_as_popup . $caption_class;
...
if ( 'yes' === $add_caption && '' !== $caption ) {
	$html .= '<figcaption class="vc_figure-caption">' . esc_attr( $caption ) . '</figcaption>';
}

$output = '
	<div class="' . esc_attr( trim( $css_class ) ) . '">
		' . wpb_widget_title( array( 'title' => $title, 'extraclass' => $caption_class_image . 'wpb_singleimage_heading' ) ) . '
