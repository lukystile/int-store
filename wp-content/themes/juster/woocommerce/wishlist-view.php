...
>
    ...
    >
        ...
        <tr>
            ...
            <th class="product-name">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product Name', 'juster' ) ) ?></span>
            </th>
            ...
            <?php if( $show_price ) : ?>
                ...
                <th class="product-price">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit Price', 'juster' ) ) ?>
                    </span>
                </th>
            ...
            <?php if( $show_stock_status ) : ?>
                ...
                <th class="product-stock-stauts">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock Status', 'juster' ) ) ?>
                    </span>
                </th>
        ...
        if( count( $wishlist_items ) > 0 ) :
                    ...
                    <tr id="yith-wcwl-row-<?php echo $item['prod_id'] ?>" data-row-id="<?php echo $item['prod_id'] ?>">
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="product-price">
                                ...
                                elseif( $product->price != '0' ) {
                                    ...
                                    echo apply_filters( 'yith_free_text', __( 'Free!', 'juster' ) );
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        ...
                        <?php if( $show_stock_status ) : ?>
                            <td class="product-stock-status">
                                ...
                                if( $stock_status == 'out-of-stock' ) {
                                    $stock_status = "Out";
                                    echo '<span class="wishlist-out-of-stock">' . __( 'Out of Stock', 'juster' ) . '</span>';
                                } else {
                                    $stock_status = "In";
                                    echo '<span class="wishlist-in-stock">' . __( 'In Stock', 'juster' ) . '</span>';
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        ...
                        <td class="product-add-to-cart">
	                        ...
	                        if( $show_dateadded && isset( $item['dateadded'] ) ):
								echo '<span class="dateadded">' . sprintf( __( 'Added on : %s', 'juster' ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
	                        endif;
	                        ?>
	                        ...
	                        <select class="change-wishlist selectBox">
		                        <option value=""><?php _e( 'Move', 'juster' ) ?></option>
		                        <?php
		                        ...
		                        ?>
			                        <option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
				                        <?php
				                        $wl_title = ! empty( $wl['wishlist_name'] ) ? esc_attr( $wl['wishlist_name'] ) : esc_attr( $default_wishlsit_title );
				                        if( $wl['wishlist_privacy'] == 1 ){
					                        $wl_privacy = __( 'Shared', 'juster' );
				                        }
				                        elseif( $wl['wishlist_privacy'] == 2 ){
					                        $wl_privacy = __( 'Private', 'juster' );
				                        }
				                        else{
					                        $wl_privacy = __( 'Public', 'juster' );
				                        }
	                        ...
	                        <?php if( $is_user_owner && $repeat_remove_button ): ?>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php _e( 'Remove this product', 'juster' ) ?>"><?php _e( 'Remove', 'juster' ) ?></a>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="product-remove">
                            <div>
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove remove_from_wishlist" title="<?php _e( 'Remove this product', 'juster' ) ?>">&times;</a>
                            </div>
                        </td>
                        <?php endif; ?>
            ...
            <tr>
                <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php _e( 'No products were added to the wishlist', 'juster' ) ?></td>
            </tr>
        <?php
        endif;
	        ...
	        <td colspan="<?php echo esc_attr( $column_count ) ?>">
	            <?php if( $show_cb ) : ?>
		            <div class="custom-add-to-cart-button-cotaniner">
		                <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php _e( 'Add the selected products to the cart', 'juster' ) ?></a>
		            </div>
	            <?php endif; ?>
	            ...
	            <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
		            <div class="ask-an-estimate-button-container">
	                    ...
	                    <?php _e( 'Ask for an estimate', 'juster' ) ?>
	                </a>
		            </div>
	            <?php endif; ?>
    ...
    <?php wp_nonce_field( 'yith_wcwl_edit_wishlist_action', 'justerh_wcwl_edit_wishlist' ); ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo $ask_estimate_url ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! empty( $additional_info_label ) ):?>
				<label for="additional_notes"><?php echo esc_attr( $additional_info_label ) ?></label>
			<?php endif; ?>
