	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('jt-each-post'); ?>>
	    <div class="jt-featured-img">
	        <div class="video-img-overlay">
	        	<?php
	        	$vid_cont = get_post_meta( $post->ID, 'video_content', true );
				$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'thumbnail_size' );
				$url = $thumb['0'];
	        	if($vid_cont) {
	        	?>
	        	<a class="popup-vimeo" href="<?php echo esc_url($vid_cont); ?>"><img src="<?php echo IMAGES; ?>/icons/format-video.png" alt=""></a><br>
	        	<?php } else { ?>
	        	<a href="<?php echo IMAGES; ?>/dummy/blog.png" class="format-image-popup jt-featured-img"><img src="<?php echo IMAGES; ?>/icons/format-video.png" alt=""></a><br>
	        	<?php } ?>
	        </div>
			<?php
			if ( has_post_thumbnail() && ! post_password_required() ) {
				...
				<img src="<?php echo IMAGES.'/dummy/blog.png'; ?>" alt="<?php esc_attr(the_title()); ?>">
			<?php } ?>
	    </div>
