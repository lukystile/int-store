...
<?php } elseif($blog_style_ot == 'blog_style_ot_4') { ?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('jt-each-post'); ?>>
		<?php
		$gal_cont = get_post_meta( $post->ID, 'gallery_content', true );
		if( $gal_cont && !post_password_required() ) {
			echo '<div class="jt-featured-img format-gallery-popup">';
			foreach( $gal_cont as $item ) {
				if ( $item['image'] ) {
				echo '<a href="'. esc_url( $item['image'] ) .'">';
				echo '<img src="'. esc_attr( $item['image'] ) .'" alt="'. esc_attr( $item['title'] ) .'">';
				echo '</a>';
				} elseif ( $item['video'] ) {
		        	echo esc_attr($item['video']);
		      	}
			}
			echo '</div>';
		} elseif( !$gal_cont && has_post_thumbnail() && !post_password_required() ) {
			$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'thumbnail_size' );
			$url = $thumb['0'];
		?>
			<a href="<?php echo esc_url($url); ?>" class="format-image-popup gall-img">
			<?php the_post_thumbnail(); ?>
			</a>
		<?php } else { ?>
			<a href="<?php echo IMAGES; ?>/dummy/blog.png" class="format-image-popup gall-img">
			<img src="<?php echo IMAGES; ?>/dummy/blog.png" alt="">
			</a>
		<?php } ?>
		<div class="jt-post-content">
			...
			<?php if( !is_single() ) { ?>
				<a href="<?php esc_url(the_permalink()); ?>" class="jt-post-read-more bg-filled"><?php echo __('Read More', 'juster'); ?></a>
			<?php }
				if(is_single()) {
				$defaults = array(
					'before'           => '<div class="wp-link-pages">' . __( 'Pages:', 'juster' ),
					'after'            => '</div>',
					'link_before'      => '<span>',
					'link_after'       => '</span>',
