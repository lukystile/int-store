...
<?php } elseif($blog_style_ot == 'blog_style_ot_4') { ?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class('jt-each-post'); ?>>
		<div class="jt-post-content">
		    <div class="jt-format-quote-icon">
		        <img src="<?php echo IMAGES; ?>/icons/format-quote.png" alt="">
		    </div>
		    <?php
		    $quote_cont = get_post_meta( $post->ID, 'quote_author_quote', true );
		    $quote_cont_author = get_post_meta( $post->ID, 'quote_author_name', true );
		    $quote_cont_author_link = get_post_meta( $post->ID, 'quote_author_link', true );
		    if($quote_cont) { ?>
			    <h3 class="jt-post-title"><?php echo esc_attr($quote_cont); ?></h3>
			    <div class="jt-format-quote">
			        <a href="<?php echo esc_url($quote_cont_author_link) ?>" target="_blank">
			        	<?php echo esc_attr($quote_cont_author); ?>
			        </a>
			    </div>
		    <?php } ?>
		</div>
